...
- CJEU (InfoCuria live index, same-day decisions): curia_search (full text matches EVERY language version at once — Czech phrases work directly; case number, case/party name, ECLI, case status closed/pending, doc_type incl. AG opinions and avis, court C/T, dates; referred_from = preliminary rulings referred by a member state's courts, e.g. ["CZ"] — works without keywords; case law ON a provision = the act's number as a phrase, query "\"2004/48\"") → curia_get_document {ecli | celex | case_number | logic_doc_id}; Czech texts usually via language:"cs".
