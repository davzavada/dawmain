...
- CJEU (InfoCuria live index, same-day decisions): curia_search (full text — all_languages extends it to every language version; case number, case/party name, ECLI, case status closed/pending, doc_type incl. AG opinions and avis, court C/T, document dates; cites_celex + cites_article = decisions citing a given act or article; referred_from = preliminary rulings referred by a member state's courts, e.g. ["CZ"]) → curia_get_document {ecli | celex | case_number | logic_doc_id}; Czech texts usually via language:"cs".
