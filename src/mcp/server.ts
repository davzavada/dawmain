...
- A CONCRETE decision cited by spisová značka / case number / ECLI: cz_caselaw_get resolves which court it belongs to ('23 Cdo 116/2017' → NS, '1 Afs 25/2024' → NSS, 'Pl. ÚS 24/10' → ÚS, 'C-311/18' → CJEU) and returns the text in ONE call — always prefer it over search + get when the user names the decision.
...
- For case law start with cz_caselaw_search — one call queries NSS + NS + Ústavní soud (and with include_eu the CJEU) in parallel; with queries + read_top it is a whole first research round in a single call. A specific decision cited by značka/ECLI → cz_caselaw_get, one call.
